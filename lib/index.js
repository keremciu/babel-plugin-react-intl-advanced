...
var FUNCTION_NAMES = ["defineMessages", "formatMessage"];
  ...
  function isFormatMessageCall(path) {
    if (t.isMemberExpression(path)) {
      ...
      if (t.isIdentifier(property) && property.node.name === "intl") {
        ...
        var objectProperty = object.get("property");
  ...
  return {
    ...
    visitor: {
      JSXOpeningElement: function JSXOpeningElement(path, state) {
        ...
        var additionalComponents = opts.additionalComponentNames || ["formatMessage"];
        var additionalComponentNameMatches = Object.keys(additionalComponents).some(function (moduleName) {
          return referencesImport(name, moduleName, additionalComponents[moduleName]);
        });

        if (referencesImport(name, moduleSourceName, COMPONENT_NAMES) || additionalComponentNameMatches) {
      ...
      CallExpression: function CallExpression(path, state) {
        ...
        var additionalFunctionNames = opts.additionalFunctionNames || ["formatMessage"];
        var additionalFunctionNameMatches = Object.keys(additionalFunctionNames).some(function (moduleName) {
          return referencesImport(callee, moduleName, additionalFunctionNames[moduleName]);
        });

        if (referencesImport(callee, moduleSourceName, FUNCTION_NAMES) || additionalFunctionNameMatches) {
          ...
          throw path.buildCodeFrameError(messagesObj);
        }

        if (isFormatMessageCall(callee)) {
          var _messagesObj = path.get("arguments")[0];
          processMessageObject(_messagesObj);
        }
      }
    }
  };

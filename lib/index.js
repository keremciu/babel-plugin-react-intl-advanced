...
function _default(_ref) {
	...
	function storeMessage(_ref7, path, state) {
		...
		if (opts.enforceDescriptions) {
			if (
				!description ||
				((0, _typeof2.default)(description) === "object" &&
	...
	return {
		...
		visitor: {
			JSXOpeningElement: function JSXOpeningElement(path, state) {
				...
				var additionalComponentNameMatches = Object.keys(additionalComponents).some(
					function(moduleName) {
			...
			CallExpression: function CallExpression(path, state) {
				...
				var additionalFunctionNameMatches = Object.keys(additionalFunctionNames).some(
					function(moduleName) {
						return referencesImport(
							callee,
							moduleName,
							additionalFunctionNames[moduleName]
						);
					}
				);

				if (
					referencesImport(callee, moduleSourceName, FUNCTION_NAMES) ||
					additionalFunctionNameMatches
				) {
